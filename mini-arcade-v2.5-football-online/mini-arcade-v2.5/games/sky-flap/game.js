...
let bird, pipes, score, best = +(localStorage.getItem('arcade_best_flap') || 0), running = false, last = 0, spawnT = 0, speed = 175, wingTick = 0, elapsed = 0, difficulty = 0;
...
function reset(){ bird={x:150,y:250,vy:0,r:16}; pipes=[]; score=0; spawnT=0; speed=175; wingTick=0; elapsed=0; difficulty=0; scoreEl.textContent=0; draw(); }
...
function flap(){ if(!running) start(); bird.vy=-(325 + difficulty*18); window.ArcadeAudio?.sfx('flap'); }
function spawn(){
  const gap=Math.round(168 - difficulty*50);
  const topMin=56, bottomReserve=125;
  const top=topMin+Math.random()*(c.height-gap-bottomReserve-topMin);
  pipes.push({x:c.width+35,w:66,top,gap,passed:false});
}
...
function loop(t){
  if(!running) return;
  const dt=Math.min(.033,(t-last)/1000); last=t; elapsed+=dt; wingTick+=dt*12; spawnT+=dt;
  difficulty=Math.min(1, elapsed/95 + score/55);
  speed=175 + difficulty*145;
  const spawnEvery=1.58 - difficulty*.38;
  if(spawnT>spawnEvery){ spawnT=0; spawn(); }
  bird.vy+=(900+difficulty*135)*dt; bird.y+=bird.vy*dt;
  pipes.forEach(p=>p.x-=speed*dt); pipes=pipes.filter(p=>p.x+p.w>-20);
  for(const p of pipes){
    if(!p.passed&&p.x+p.w<bird.x){ p.passed=true; score++; scoreEl.textContent=score; window.ArcadeAudio?.sfx('point'); }
    if(bird.x+bird.r>p.x&&bird.x-bird.r<p.x+p.w&&(bird.y-bird.r<p.top||bird.y+bird.r>p.top+p.gap)) return over();
  }
  if(bird.y+bird.r>c.height-24||bird.y-bird.r<0) return over();
