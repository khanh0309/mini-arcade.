...
let chicken, cars, score, crossings, best=+(localStorage.getItem('arcade_best_chicken')||0), running=false, last=0, touchStart=null, bounce=0;
...
function reset(){ chicken={col:5,row:9}; score=0; crossings=0; cars=[]; bounce=0; laneCfg.forEach((cfg,i)=>{ for(let pos=-cfg.gap; pos<c.width+cfg.gap; pos+=cfg.gap){ cars.push({lane:i,row:lanes[i],x:pos+(i%2)*110,w:80+(i%3)*12,h:42,cfg}); } }); scoreEl.textContent=0; crossEl.textContent=0; draw(); }
...
function chickenRect(){ return {x:chicken.col*CELL+14,y:chicken.row*CELL+12,w:37,h:42}; }
...
function loop(t){ if(!running) return; const dt=Math.min(.033,(t-last)/1000); last=t; bounce+=dt*12; const multi=1+crossings*.12; for(const car of cars){ car.x+=car.cfg.dir*car.cfg.speed*multi*dt; if(car.cfg.dir>0&&car.x>c.width+100) car.x=-car.w-120; if(car.cfg.dir<0&&car.x+car.w<-100) car.x=c.width+120; } const cr=chickenRect(); for(const car of cars){ const y=car.row*CELL+(CELL-car.h)/2; if(car.row===chicken.row&&hit(cr,{x:car.x,y,w:car.w,h:car.h})) return over(); } draw(); requestAnimationFrame(loop); }
