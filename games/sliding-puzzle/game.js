...
function win(){if(!best||moves<best){best=moves;localStorage.setItem('arcade_best_puzzle',best);bestEl.textContent=best}window.ArcadeAudio?.sfx('win');winText.textContent=`Bạn hoàn thành trong ${moves} bước. Kỷ lục: ${best} bước.`;overlay.classList.remove('hidden');window.ArcadeLeaderboard?.show('sliding-puzzle',moves,{title:'Sliding Puzzle',sort:'asc',suffix:' bước'})}
